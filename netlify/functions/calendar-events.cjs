...
const { categorizeEvent, parsePriceFromDescription, parseSpotsFromDescription, enrichEventWithRegistrationData } = require('./lib/calendarUtils.cjs');
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (syncToken) {
      ...
      if (isNaN(monthNum) || monthNum < 1 || monthNum > 12 || isNaN(yearNum) || yearNum < 2020 || yearNum > 2100) {
        return {
          statusCode: 400,
          headers,
          body: JSON.stringify({ error: 'Invalid month or year parameter' }),
        };
      }
    ...
    // Google Calendar API returns max 250 per page; iterate until all pages consumed.
    let events = [];
    ...
    do {
      ...
      if (nextPageToken) pageParams.pageToken = nextPageToken;

      const response = await calendar.events.list(pageParams);
    ...
    return {
      ...
      headers: {
        ...headers,
        'Cache-Control': 'public, max-age=15, s-maxage=15, stale-while-revalidate=10',
      },
      body: JSON.stringify({
        events: events,
        total: events.length,
